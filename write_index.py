...
<body>
  ...
  <div class="leaderboard-box">
    <div class="lb-header">
      <div class="lb-title">🏆 Leaderboard — Top Scores</div>
  ...
  <script>
    ...
    function updateLeaderboard(board, rank, bestScore) {
      ...
      list.innerHTML = board.map((entry, i) => {
        const posClass = i < 3 ? medals[i] : 'other';
        ...
        return '<div class="lb-row">' +
          ...
          '<div style="text-align:right">' +
            ...
            '<div class="lb-time">'+entry.timestamp+'</div>' +
          '</div>' +
        '</div>';
      }).join('');
    }
    ...
    function estimatePosture(p){
      try{
        ...
        if(!postureCalibrated){
          faceHeightSamples.push(faceHeight);calibrationFrames++;
          if(calibrationFrames>=30){baselineFaceHeight=faceHeightSamples.reduce((a,b)=>a+b)/faceHeightSamples.length;postureCalibrated=true;}
          return 100;
        }
        let score=100;
